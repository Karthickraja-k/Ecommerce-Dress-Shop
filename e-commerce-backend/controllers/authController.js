...
exports.forgotPassword = catchAsyncError( async (req, res, next)=>{
    ...
    try{
        sendEmail({
            email: user.email,
            subject: "CRESCENT INFO TECH Password Recovery",
            message
        })
    ...
    if (cartTotal >= 500){
        var transporter = nodemailer.createTransport({
          ...
          var mailOptions = {
            ...
            text: `Hello ${adminUser.name}! /n` + 
            `${user.email} has been purchase above 500 rupees.`
          };
...
/* exports.sendNewsletter =  async (req, res) => {
  ...
  const sendNewsletter = async (subject, content, recipients) => {
    ...
    const mailOptions = {
      from: adminUser.email, // Replace with your Gmail email
      to: recipients.join(', '),
      subject: subject,
      html: content,
    };
