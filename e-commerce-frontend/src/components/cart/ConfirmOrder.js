...
export default function ConfirmOrder() {
    ...
    // Calculate itemsPrice using offerPrice if it exists
    const itemsPrice = cartItems.reduce((acc, item) => 
        acc + (item.offerPrice ? item.offerPrice : item.price) * item.quantity, 
        0
    );
                ...
                <div className="col-12 col-lg-8 mt-5 order-confirm">
                    ...
                    {cartItems.map(item => (
                        <Fragment key={item.product}>
                            <div className="cart-item my-1">
                                <div className="row">
                                    <div className="col-4 col-lg-2 ml-5">
                                        <img src={item.image} alt={item.name} height="45" width="65" />
                                    </div>
                                    ...
                                    <div className="col-4 col-lg-4 mt-4 mt-lg-0 ml-5">
                                        <p>
                                            {item.quantity} x ₹{item.offerPrice ? item.offerPrice : item.price} = 
                                            <b> ₹{(item.quantity * (item.offerPrice || item.price)).toFixed(2)}</b>
                                        </p>
                                    </div>
                                </div>
                            </div>
